...
public class Play extends GameScene {
    ...
    private String getWinner() {
        int hp1 = this.frameData.getCharacter(true).getHp();
        int hp2 = this.frameData.getCharacter(false).getHp();

        if (hp1 > hp2) return "P1";
        if (hp2 > hp1) return "P2";
        return "DRAW";
    }
    ...
    private void processingRoundEnd() {
        ...
        if (FlagSetting.slowmotion) {
            ...
            if (this.endFrame > GameSetting.ROUND_EXTRAFRAME_NUMBER) {
                ...
                // ---20260104 yunoki add_2---
                String winner = getWinner();

                Logger.getAnonymousLogger().log(
                        Level.INFO,
                        "[RoundEnd] Round=" + this.currentRound + " Winner=" + winner
                );
